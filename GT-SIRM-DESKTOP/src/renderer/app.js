...
function drawBgTransition(ctx, srcCur, srcNext, alpha, W, H, type, softness) {
  ...
  // slides: تَحريك + عند softness نَستَخدِم gradient mask عَلى حَافّة الـseam لِلقادِم
  if (type.startsWith("slide")) {
    ...
    // اِرسم الحاليّ في مَوقعه (بلا شَفافيّة إضافيّة — النُعومة عَلى القادِم)
    ctx.save();
    ctx.globalAlpha = 1;
    ctx.translate(dx, dy);
    drawSrc(srcCur);
    ...
    // v1.2 fix — نُعومة حَقيقيّة لِلـslides عبر gradient mask عَلى seam القادِم
    const { canvas: offCanvas, ctx: octx } = _getSoftMaskCanvas(W, H);
    octx.setTransform(1, 0, 0, 1, 0, 0);
    octx.clearRect(0, 0, W, H);
    octx.globalAlpha = 1;
    octx.globalCompositeOperation = "source-over";
    // اِرسم القادِم في مَوقعه على الـoff-canvas
    octx.save();
    octx.translate(dxNext, dyNext);
    imgCover(octx, srcNext, 0, 0, W, H);
    octx.restore();
    // gradient على الـseam يَجعَل حافّة القادِم تَتَلاشى نَحو الحاليّ
  ...
  // ارسم الـmask بـblur — يُنتِج حَافّة feathered
