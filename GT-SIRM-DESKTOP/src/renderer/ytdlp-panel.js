...
async function runYtdlpDownload() {
  ...
  window.SIRM.onYtdlpProgress(({ line }) => appendYtdlpLog(line));

  try {
    const result = await window.SIRM.ytdlpDownload({ url, type });
...
function applyDownloadedBackground(filePath, type) {
  ...
  if (type === "video") {
    // تطبيق كخلفية فيديو — نفس منطق GT-SIRM
