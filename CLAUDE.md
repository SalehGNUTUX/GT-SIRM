...
│   ├── platform-io.js                 ← v1.2.1 حَفظُ المِلَفّاتِ + اليَقَظة (window.PIO)
│   ├── android-src/                   ← ⭐ المَصادِرُ الأَصليّةُ المُتَتَبَّعة (v1.2.1)
│   │   ├── AndroidManifest.xml            ← المَصدَرُ المُعتَمَد (لا تُعَدِّل android/…)
│   │   └── java/org/gnutux/gtsirm/
│   │       ├── MainActivity.java          ← يُسَجِّلُ GtsirmNative
│   │       ├── GtsirmNative.java          ← MediaStore + keepAwake + الخِدمة
│   │       └── ExportService.java         ← خِدمةُ مُقَدِّمة + PARTIAL_WAKE_LOCK
│   ├── android/                       ← Capacitor 6 project (gitignored, generated)
│   │   └── app/src/main/
...
### ⚠️ `android/` كامِلاً في `.gitignore` — المَصدَرُ في `android-src/` (v1.2.1)
`git ls-files GT-SIRM-WEB/android` تُعيدُ **صِفراً**. كُلُّ ما يُكتَبُ في `android/app/src/main/`
مُعَرَّضٌ لِلضَياعِ عِندَ `cap add` أو استِنساخٍ جَديد — وهذا ما كانَ يَحدُثُ لِـ`AndroidManifest.xml`
المُخَصَّصِ قَبلَ v1.2.1 (الأُذوناتُ وintent-filter لِـ`.gtsirm` كانَت مَحَلّيّةً فَقَط).
**القاعِدة:** عَدِّل `android-src/` وَحدَه؛ `build-apk.sh` يَنسَخُهُ فَوقَ المُوَلَّدِ بَعدَ `cap sync`
(كَما يَفعَلُ `gen-android-icons.py` لِلأَيقونات).

### `<a download>` لا يَفعَلُ شَيئاً داخِلَ WebView (v1.2.1)
لا مُديرَ تَنزيلاتٍ في Capacitor WebView: النِداءُ يَمضي بِلا أَثَرٍ **ولا خَطَأ**، فَيَظُنُّ
المُستَخدِمُ أنَّهُ حَفِظَ ولا يَجِدُ شَيئاً. أَصابَ هذا الفيديو المُصَدَّرَ وحِفظَ `.gtsirm` مَعاً.
**لا تُسَلِّم مَلَفّاً بِـ`a.click()` مُباشَرةً** — مُرَّ بِـ`PIO.deliverFile()` في `platform-io.js`
(سِلسِلة: MediaStore ← Capacitor FS ← FSA ← تَنزيل)، واحتَفِظ بِالـblob لِزِرِّ حَفظٍ يَدَويّ.
قِطَعُ base64 يَجِبُ أن تَقبَلَ القِسمةَ عَلى 3 وإلّا ظَهَرَت حَشوةُ `=` وَسَطَ المَجرى فَفَسَدَ المَلَفّ.

### `setTimeout` مَخنوقٌ في الخَلفيّة — لا تَستَعمِلهُ في حَلقةِ التَصدير (v1.2.1)
حَلقةُ `startWebExportV2` كانَت تَستَعمِلُ `setTimeout(5)` لِضَبطِ ضَغطِ المُرَمِّز. حينَ تُخفى
الصَفحةُ (شاشةٌ مُطفَأة / بَرنامَجٌ آخَر) تُخنَقُ المُؤَقِّتاتُ إلى نِداءٍ كُلَّ ثانية، فَيَهبِطُ
التَصديرُ إلى ~إطارٍ في الثانِية — وهُوَ ما بَدا لِلمُستَخدِمِ «تَوَقُّفاً».
البَديل: حَدَثُ `dequeue` لِلطابور (مَعَ حارِسٍ زَمَنيٍّ لِئَلّا يُعَلَّقَ التَصديرُ لَو تَعَثَّرَ
المُرَمِّز) + `PIO.yieldToBrowser()` المَبنيُّ عَلى `MessageChannel`. كِلاهُما لا يُخنَق.
وفي Android لا يَكفي هذا وَحدَه: يَلزَمُ `ExportService` (خِدمةُ مُقَدِّمة + `PARTIAL_WAKE_LOCK`).
⚠️ يَبقى فَكُّ تَرميزِ `<video>` مُعَلَّقاً حينَ يُغادِرُ المُستَخدِمُ البَرنامَجَ فِعلاً — لِذا
`FLAG_KEEP_SCREEN_ON` هُوَ خَطُّ الدِفاعِ الأَوَّلُ لِمَشاريعِ خَلفيّةِ الفيديو.

### البَسمَلة: النَصُّ يَضُمُّها والصَوتُ لا (v1.2.1)
`api.alquran.cloud/quran-uthmani` يُدمِجُ البَسمَلةَ في نَصِّ الآيةِ 1 لِكُلِّ سورة، عَدا
**الفاتِحة** (آيةٌ مُستَقِلّةٌ بِرَقمِ 1) و**بَراءة** (لا بَسمَلة). أمّا `SSS001.mp3` في everyayah
فَيَحوي الآيةَ وَحدَها. النَتيجةُ قَبلَ الإصلاح: تُكتَبُ ولا تُتلى.
`applyBasmalaSlice` تَفصِلُها شَريحةً صَوتُها `001001.mp3` عَبرَ `audioSurah`/`audioAya`.
**كُلُّ عُنوانِ صَوتٍ يَجِبُ أن يَمُرَّ بِـ`ayaAudioUrl(folder, surahNum, aya)`** لا
`buildAudioUrl(..., aya.numberInSurah)` مُباشَرةً، وإلّا فَقَدَتِ البَسمَلةُ صَوتَها.
المُطابَقةُ مُطَبَّعةٌ عَبرَ `normalizeWithMap` (تَشكيلٌ مُختَلِفٌ بَينَ المَصادِر) — تُحُقِّقَ
عَلى 112 سورةً بِلا فَقد.

### عَناصِرُ واجِهةٍ مَيِّتة — تَحَقَّق قَبلَ أن تُصَدِّق (v1.2.1)
`<select id="export-res">` كانَ في الواجِهةِ مُنذُ الاشتِقاقِ ولا يَقرَؤُهُ أَيُّ سَطر: يَختارُ
المُستَخدِمُ 720p فَيُصَدَّرُ بِـ1080p. الأَبعادُ كانَت تَأتي مِن `FMT_SIZES`/`PRESETS` وَحدَها.
الآنَ سَقفُ دِقّةٍ حَقيقيٌّ عَبرَ `computeCanvasSize()`/`applyCanvasSize()` — يُشتَقُّ دائِماً مِن
...
### `willReadFrequently: true` يُلغي تَسريعَ العَتادِ (v1.2.3) — أَكبَرُ مَكسَبِ أَداء
اللَوحةُ كانَت تُنشَأُ دائِماً بِهذا العَلَم، فَيُبقيها المُتَصَفِّحُ في ذاكِرةِ المُعالِجِ
ويُلغي التَسريع: كُلُّ `drawImage` وتَدَرُّجٍ و`shadowBlur` (18 مَوضِعاً) ونَصٍّ عَرَبيٍّ
يُرسَمُ بِالبَرمَجيّاتِ وَحدَها. **قِياسٌ مِن هاتِفِ المُستَخدِم: 82.2ms لِلإطار مِن أَصلِ
120.5ms (68%)**.
القِراءةُ بِالبِكسِل لا يَحتاجُها إلّا `applyColorFilter` (عِندَ نَمَطِ لَونٍ غَيرِ «بِدون»)
و`drawGrain` والمُؤَثِّراتُ الثَمانِيةُ الثَقيلة. `ensureCanvasMode()` تَختارُ الوَضعَ
تِلقائيّاً. الخاصيّةُ **تُثَبَّتُ عِندَ أَوَّلِ `getContext` ولا تُغَيَّرُ**، فَالسَبيلُ
الوَحيدُ استِبدالُ عُنصُرِ `<canvas>` (آمِنٌ هُنا: لا مُستَمِعَ أَحداثٍ عَلَيه).
⚠️ لا تُبَدِّلِ اللَوحةَ أثناءَ التَصدير: المُحَرِّكُ يُمسِكُ المَرجِعَ لِـ`VideoFrame`.

### التَنازُلُ عَنِ المُعالِجِ لِكُلِّ إطارٍ يُكَلِّفُ تَركيبَ اللَوحةِ كامِلةً (v1.2.3)
`PIO.yieldToBrowser()` بَعدَ كُلِّ إطارٍ كَلَّفَ **36ms لِلإطار (30% مِنَ الزَمَن)** في
قِياسِ المُستَخدِم: كُلُّ تَنازُلٍ يَسمَحُ لِلمُتَصَفِّحِ بِتَركيبِ لَوحةِ 720×1280 عَلى
الشاشة. اخنُقهُ زَمَنيّاً (كُلَّ 120ms) — يَكفي لِبَقاءِ الواجِهةِ حَيّةً.

### «أَطلِق وانسَ» يُجَمِّدُ الفيديو، والصَوابُ «تَقَدَّم عِندَ الجاهِزيّة» (v1.2.3)
إسنادُ `video.currentTime` في كُلِّ إطارٍ **يُلغي النَقلةَ التي قَبلَه**، فَلا تَكتَمِلُ
نَقلةٌ واحِدةٌ وتَتَجَمَّدُ الخَلفيّةُ عَلى أَوَّلِ إطار (اِرتِدادُ v1.2.2). الصَوابُ:
عَلَمٌ `_sirmSeeking` يَمنَعُ طَلَبَ نَقلةٍ جَديدةٍ قَبلَ انتِهاءِ الجارية، مَعَ حارِسٍ
زَمَنيٍّ لَو ضاعَ حَدَثُ `seeked`.

### H.264 Baseline كانَ الخِيارَ الوَحيد (v1.2.3)
`videoTries` لَم تَحوِ إلّا `avc1.42E01F` — أَدنى مِلَفّاتِ H.264 جَودةً عِندَ نَفسِ
مُعَدَّلِ البِتّ. أُضيفَ High (`avc1.640028`) ثُمَّ Main قَبلَه. و`latencyMode:"quality"`
مَعَ سِلسِلةِ سُقوطٍ إلى إعدادٍ أَدنى لَو رَفَضَ مُحَرِّكٌ أَقدَمُ الحُقولَ الحَديثة
(وإلّا فَشِلَ التَصديرُ كامِلاً).

### الافتِراضيّاتُ تُقرَأُ مِن HTML لا مِن جَدوَلٍ مُوازٍ (v1.2.3)
`defaultValue` / `defaultChecked` / `defaultSelected` تَبقى كَما في سِماتِ الصَفحةِ ولا
تُغَيِّرُها `restoreAllSettings` (تَكتُبُ `.value` لا السِمة). فَزِرُّ ↺ لا يَحتاجُ جَدوَلَ
افتِراضيّاتٍ يُنسى تَحديثُه، وكُلُّ إعدادٍ جَديدٍ يَنالُ الميزةَ تِلقائيّاً.
المَجموعاتُ المُوَلَّدةُ بِـJS (الخُطوطُ والقُرّاء) بِلا سِمةِ `checked` → افتِراضِيُّها
أَوَّلُ خِيار.

### لا مَسارَ حَفظٍ بِلا `try/catch` (v1.2.3)
`saveProjectInteractive` كانَت بِلا حِمايةٍ في كُلِّ سِلسِلَتِها: خَطَأٌ في تَسَلسُلِ
الأُصولِ (أو نَفادُ ذاكِرةٍ مَعَ فيديو كَبير) يَرفُضُ الوَعدَ **بِصَمتٍ تامّ** — يَنقُرُ
المُستَخدِمُ 💾 فَلا يَحدُثُ شَيءٌ ولا رِسالة. وتَضمينُ الأُصولِ يُنشِئُ ثَلاثَ نُسَخٍ
في الذاكِرة (base64 ← JSON ← base64 لِلكِتابةِ الأَصليّة)، فَحَدُّ الهاتِفِ 12 م.ب لا 50.

### اختبارُ نُسخةِ الويبِ في Electron: امسَحِ الـService Worker (v1.2.3)
